...
class DataBowl3Classifier(Dataset):
    def __init__(self, root_path, phase='train', isAugment=True):
        ...
        # CSV lives one level above the split folder (e.g. data/NSCLC_PROCESSED.CSV)
        label_path = os.path.join(os.path.dirname(root_path), "NSCLC_PROCESSED.CSV")

        data_frame = pandas.read_csv(label_path)
        # age is already normalised to [0,1] by preprocess_nsclc.py — no raw age column
